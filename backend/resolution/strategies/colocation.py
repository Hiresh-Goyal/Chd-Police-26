...
def execute(conn: Connection, case_id: str) -> dict:
    ...
    ent_stmt = select(entities.c.id, entities.c.canonical_id).where(
        entities.c.case_id == case_id,
        entities.c.entity_type == 'PHONE'
    )
    ent_rows = conn.execute(ent_stmt).fetchall()
    phone_to_ent_id = {r.canonical_id: r.id for r in ent_rows}
    ...
    for i in range(len(rows)):
        for j in range(i + 1, len(rows)):
            ...
            if r1.location_raw == r2.location_raw:
                if abs((datetime.fromisoformat(r1.ts_start.replace("Z", "+00:00")) - datetime.fromisoformat(r2.ts_start.replace("Z", "+00:00"))).total_seconds()) <= 30 * 60:
                    ...
                    if phone1 != phone2:
                        ...
                        if ent1 and ent2:
                            new_links.append({
                                ...
                                "confidence": 0.70,
                                "evidence_event_ids": json.dumps([r1.id, r2.id])
                            })
                            links_created += 1
