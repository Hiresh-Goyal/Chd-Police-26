...
async def get_case_files(case_id: str):
    """Get all uploaded files for a specific case."""
    try:
        from backend.db.connection import get_connection
        ...
        with get_connection() as conn:
            rows = conn.execute(
                raw_files_table.select().where(raw_files_table.c.case_id == case_id)
            ).fetchall()
            return [
                {
                    "id": row.id,
...
async def analyze_case(case_id: str, response: Response):
    ...
    # If the engine fails or is unavailable, return empty results
    return {
        "case_id": case_id,
        "findings": [],
        "episodes_created": 0,
