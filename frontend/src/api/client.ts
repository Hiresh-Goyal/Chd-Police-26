...
// Files
export const getCaseFiles = async (caseId: string): Promise<any[]> => {
  const res = await api.get<any[]>(`/cases/${caseId}/files`);
  return res.data;
};
...
export const getAuditLogs = async (): Promise<any[]> => {
  const res = await api.get<any[]>('/system/audit');
  return res.data;
};

export const getSentinelItems = async (): Promise<any[]> => {
  const res = await api.get<any[]>('/system/sentinel');
  return res.data;
};
