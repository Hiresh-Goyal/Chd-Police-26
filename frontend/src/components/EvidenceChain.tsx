...
import { ChevronRight, ChevronDown, AlertTriangle, FileText } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 text-sm">
      ...
      <div className="ml-2 border-l border-slate-700 pl-4 space-y-2">
        {events.length > 0 ? events.map(renderEventRow) : (
          <div className="text-slate-500 text-xs">No canonical evidence events are linked to this finding.</div>
        )}
      </div>
    </div>
  );
