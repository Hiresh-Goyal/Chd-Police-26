...
export const AnalysisNav: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-[#D9E1EA] rounded-lg px-2 flex items-center overflow-x-auto hide-scrollbar shadow-[0_2px_4px_rgba(11,35,64,0.02)] shrink-0 select-none">
      <NavLink to={`/cases/${caseId}`} end className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">work</span>
        <span>Case Workspace</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/upload-evidence`} className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">upload_file</span>
        <span>Upload Evidence</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/timeline`} className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">timeline</span>
        <span>Timeline</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/entity-graph`} className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">hub</span>
        <span>Entity Graph</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/geospatial`} className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">map</span>
        <span>Geospatial Map</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/criminal-flow`} className={tabClass}>
        <span className="material-symbols-outlined text-[18px]">account_tree</span>
        <span>CriminalFlow</span>
      </NavLink>

      <NavLink to={`/cases/${caseId}/evidence-report`} className={tabClass}>
