...
function formatISTDateTime(value: any): string {
  if (!value) return '—';
...
function generateReportHTML(report: any, sections: ReportSectionItem[], certOfficer: string): string {
  ...
  sectionBlocks.sec_3 = `<h2>3. Cross-Domain Chronological Timeline</h2>${report.timeline.length ? `<table><tr><th>Time</th><th>Domain</th><th>Event</th></tr>${report.timeline.map((e:any)=>`<tr><td>${esc(formatISTDateTime(e.ts_start))}</td><td>${esc(e.domain ?? e.event_type)}</td><td>${esc(e.title ?? e.event_type)} — ${esc(e.actor_raw)}${e.peer_raw ? ` → ${esc(e.peer_raw)}` : ''}${e.amount != null ? ` — ${money(e.amount)}` : ''}</td></tr>`).join('')}</table>` : '<p>No timeline events returned by the backend.</p>'}`;
  const riskEntities = report.graph.nodes.filter((n:any) => Number(n.risk_score ?? 0) > 0);
  sectionBlocks.sec_4 = `<h2>4. Entity Link Analysis &amp; Multi-Domain Associations</h2>${riskEntities.length ? `<table><tr><th>Entity</th><th>Role</th><th>Domain</th><th>Risk Score</th></tr>${riskEntities.map((n:any)=>`<tr><td>${esc(n.label ?? n.canonical_value)}</td><td>${esc(n.role)}</td><td>${esc(n.domain)}</td><td>${esc(n.risk_score)} / ${esc(n.risk_level)}</td></tr>`).join('')}</table>` : '<p>No risk-bearing entities returned by the backend.</p>'}`;
