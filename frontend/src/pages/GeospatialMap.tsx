...
export const GeospatialMap: React.FC = () => {
  ...
  const [geoPoints, setGeoPoints] = useState<GeoLocationNode[]>(GEO_POINTS);
  const [trajectoryPath, setTrajectoryPath] = useState<[number, number][]>(GEO_POINTS.map(p => [p.lat, p.lng]));
  ...
  const loadGeospatialData = async () => {
    try {
      setIsLoading(true);
      const data = await apiClient.getGeospatial(caseId!);

      const mapped = data.map(evt => {
        let domain: 'CDR' | 'IPDR' | 'BANK' = 'CDR';
        let color = '#0891B2';
  ...
  const [selectedPoint, setSelectedPoint] = useState<GeoLocationNode | null>(geoPoints[0] || null);
              ...
              <div className="space-y-1.5">
                {geoPoints.map((pt, idx) => (
                  <button key={pt.id} onClick={() => setSelectedPoint(pt)}
                    className={`w-full text-left p-2 rounded border text-[11px] transition-colors ${selectedPoint?.id === pt.id
