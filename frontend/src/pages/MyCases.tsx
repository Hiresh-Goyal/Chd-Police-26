...
export const MyCases: React.FC = () => {
  ...
  const handleCreateCase = async (e: React.FormEvent) => {
    ...
    try {
      const title = `${newCaseType} — ${newCaseSubject}`;
      const newCase = await apiClient.createCase(title, 'New investigation');
      setIsNewCaseModalOpen(false);
      setNewCaseSubject('');
      showToast(`Case created — upload evidence to begin analysis.`, 'success');
      navigate(`/cases/${newCase.id}/upload-evidence`);
    } catch (err) {
  ...
  const handleExportCSV = () => {
    const csvContent =
      'Case ID,Title,Status,Last Activity\n' +
      filteredCases.map(c => `"${c.id}","${c.title}","${c.status}","${c.updated_at}"`).join('\n');
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-ds-muted">Loading cases...</div>;
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      {viewMode === 'list' ? (
                ...
                {filteredCases.map(c => {
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-[#64748B] text-xs font-mono">{new Date(c.updated_at).toLocaleString()}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCases.map(c => {
            const isPrimary = false; // We can't rely on '2847' anymore
            return (
              <div
                key={c.id}
                onClick={() => navigate(`/cases/${c.id}`)}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono font-bold text-sm text-[#0B5CAB]">#{c.id.substring(0, 8)}</span>
                  </div>
                  <h3 className="font-semibold text-[#191C1E] text-base mb-1">{c.title}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs border-t border-b border-[#D9E1EA]/60 py-2.5 my-2">
                    <div>
                      ...
                      <span className="font-mono text-[#191C1E]">{new Date(c.updated_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
