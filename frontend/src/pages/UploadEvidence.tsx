...
export const UploadEvidence: React.FC = () => {
    ...
    try {
      ...
      if (data.files) {
        const loadedFiles = data.files.map(f => ({
          ...
          status: 'complete',
          progress: 100,
          recordsCount: f.events_created,
          hash: 'backend_processed',
          uploadDate: new Date().toLocaleString(),
          size: 'Unknown'
        }));
        setQueue(loadedFiles);
      }
    } catch (err) {
      showToast('Failed to load case files', 'error');
    }
  };
  ...
  const handleFilesAdded = async (files: FileList | null) => {
    ...
    for (let i = 0; i < fileArray.length; i++) {
      ...
      try {
        await apiClient.uploadFile(caseId, file, domain);
        showToast(`Uploaded ${file.name} successfully`, 'success');
        // Reload actual files from backend
        await loadFiles();
      } catch (err) {
        showToast(`Failed to upload ${file.name}`, 'error');
        setQueue(prev => prev.map(q => q.id === tempId ? { ...q, status: 'error', progress: 0 } : q));
      }
    }
  };
  ...
  const handleCopyHash = (hash: string) => {
    ...
    showToast('Evidence hash copied to clipboard.', 'success');
  };
  ...
  const handleRetry = (id: string) => {
    // Retry not strictly supported in backend via UI yet, just mock refresh
    loadFiles();
  };
